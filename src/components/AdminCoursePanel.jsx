...
export default function AdminCoursePanel() {
  ...
  const fetchCourses = async () => {
    ...
    const sortedCourses = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }))
  ...
  const saveEdit = async () => {
    const db = getDb();
    await updateDoc(doc(db, 'courses', editingId), editFields);
  ...
  return (
    <div>
      ...
      <div className="space-y-2 mb-4">
        ...
        <input className="border p-2 rounded w-full" placeholder="골프장 링크" value={link} onChange={(e) => setLink(e.target.value)} />
        <button onClick={addCourse} className="bg-blue-500 text-white px-4 py-2 rounded">추가</button>
      </div>
          ...
          <li key={c.id} className="border p-4 rounded shadow">
            {editingId === c.id ? (
              ...
              <>
                ...
                {c.link && <div className="text-sm">🔗 <a href={c.link} className="text-blue-500 underline" target="_blank">웹사이트</a></div>}
